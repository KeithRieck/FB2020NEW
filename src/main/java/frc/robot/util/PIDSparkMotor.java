...
public class PIDSparkMotor implements MotorController {
	...
	/**
	 * This class uses PID for driving at a specified velocity.
	 */
	...
	/** Secondary PID configuration is used for driving to a position. */
	...
	/**
	 * @param b True if you want PID to be enabled for driving to specified velocity.
	 */
	...
	public void driveToPosition(double inches) {
		...
		if (pidController.setReference(setPointPosition, CANSparkMax.ControlType.kPosition, SECONDARY_SLOT) != REVLibError.kOk) {
	...
	public void set(double speed) {
		...
		if (closedLoop) {
			...
			if (pidController.setReference(setPointVelocity, CANSparkMax.ControlType.kVelocity) != REVLibError.kOk) {
				System.out.println("ERROR: Failed to set setpoint on " + this);
			}
		} else {
			motor.set(speed);
